...
public class RenderSystem extends EntitySystem{
    ...
    private void drawHealthbar(HealthComponent healthComp , PositionComponent pos){
    	float healthbarHeight = 1.2f;
    	float healthbarWidth = 16;
    	// getting the scale ratio
    	float ratio = healthbarWidth / healthComp.maxHealth;
    	float currentWidth = healthComp.health * ratio;
		// red background
        sr.set(ShapeType.Filled);
        sr.setColor(1f, 0,0,1f);
        sr.rect(pos.x + healthbarWidth / 2, pos.y , healthbarWidth , healthbarHeight);
        // green current health
        sr.setColor(0, 1f , 0 , 1f);
        sr.rect(pos.x + healthbarWidth / 2, pos.y, currentWidth , healthbarHeight);
    }
}
